...
class Countly {
  ...
  updateQueueRequest = (url, data) =>
    new Promise(async (resolve, reject) => {
      ...
      const newData = this.addDefaultParameters(data);
      newData.app_key = this.APP_KEY;
      const isLengthExceedGetLimit = this.checkLength(newData);
      let newURL = null;
      if (this.secretSalt) {
  ...
  registerPush = token => {
    if (!this.isToken) {
      ...
      this.get("/i", data, result => {
        this.log("registerPush", result);
      });
    }
  };
